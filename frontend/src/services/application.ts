...
export const getApplicationList = async(searchValue: string, page: number, perPage:number) => {
  const response = await apiClient.get('/api/v1.0/accounts/applications', { params: { search_value: searchValue, page, per_page:perPage}});
