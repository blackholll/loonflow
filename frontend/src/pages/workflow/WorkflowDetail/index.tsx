...
import {Table, Form, Card, Row, Col, Input, Tooltip, Select, Switch, Button, Collapse, message, Tabs} from "antd";
...
class WorkflowDetail extends Component<any, any> {
  ...
  componentDidMount() {
    ...
    if (workflowId) {
      this.setState({workflowId}, ()=>this.fetchWorkflowDetail());
    } else {
      // this.setState({workflowId, panelActiveKey: 'workflowBasicInfo'})
      this.setState({workflowId, panelActiveKey: '1'})
    }
  }

  fetchOptionalFieldList = async () => {
    const basicFieldList = [];
    if (this.state.workflowId){
      // todo: get field and add to basicfieldlist
  ...
  fetchWorkflowDetail = async() => {
    const result = await getWorkflowDetail(this.state.workflowId);
    if (result.code !==0) {
      message.error(`获取工作流基础信息失败: ${result.msg}`);
    } else {
      this.setState({workflowDetailResult: result.data});
    }
  }
  ...
  getWorkflowDetailField = (fieldName:string) =>{
    if(this.state && this.state.workflowDetailResult && this.state.workflowDetailResult[fieldName]){
      if (fieldName === 'notices') {
        return this.state.workflowDetailResult[fieldName].split(',');
      }
      return this.state.workflowDetailResult[fieldName]
    }
    return ''
  }

  getWorkflowInitialValues = () => {
    const result = this.state.workflowDetailResult;
  ...
  panelChange = (key: string) => {
    console.log(key);
    const newKey = -Number(this.state.panelActiveKey);
    this.setState({panelActiveKey: newKey})
  }
  ...
  render() {
    ...
    return (
      <Card>
        <Collapse accordion activeKey={this.state.panelActiveKey} onChange={this.panelChange} bordered={false}>
