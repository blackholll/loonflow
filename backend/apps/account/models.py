...
class PersonalAccessToken(BaseCommonModel):
    """
    User-issued personal access token for API/MCP clients.
    The secret is never stored in plaintext; secret_ciphertext holds AES-encrypted
    Django password hasher output of the secret segment only.
    """
    ...
    token_label = models.CharField("token_label", max_length=200, blank=True, default="")
    secret_ciphertext = models.TextField("secret_ciphertext", blank=True, default="")
    mask_prefix = models.CharField("mask_prefix", max_length=64, blank=True, default="")
    expires_at = models.DateTimeField("expires_at", null=True, blank=True)
    last_used_at = models.DateTimeField("last_used_at", null=True, blank=True)
    revoked_at = models.DateTimeField("revoked_at", null=True, blank=True)
